...
Dependents are classified by file type (source, test, example) so the UI
can present them in clearly separated groups.
...
class DependentInfo(TypedDict):
    """Information about a single dependent file."""
...
class ImpactReport(TypedDict):
    """Full impact analysis report for a target file."""
...
def make_relative(abs_path: str, repo_path: str) -> str:
    ...
    try:
        return str(
            Path(abs_path).relative_to(Path(repo_path).resolve())
        ).replace("\\", "/")
    except ValueError:
        try:
            return str(
                Path(abs_path).relative_to(Path(repo_path))
            ).replace("\\", "/")
        except ValueError:
...
def analyze_impact(
    ...
    criticality, classifies them by file type, and determines
...
def format_impact_report(report: ImpactReport) -> str:
    ...
    if report["critical_dependents"]:
        for dep in report["critical_dependents"]:
            ...
            file_type = dep.get("file_type", "source")
            tag = f" [{file_type}]" if file_type != "source" else ""
            reasons = ", ".join(dep["reasons"]) if dep["reasons"] else ""
            lines.append(f"    [{dep['score']:.1f}]  {label:<35s}{tag} {reasons}")
    else:
        lines.append("    (none)")
