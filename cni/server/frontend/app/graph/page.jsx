...
import { ZoomIn, ZoomOut, Maximize, Expand, Shrink, Lock, Unlock, Camera, Search, Sparkles, X, Route, ArrowRight, RefreshCw, Download, Star, PanelLeftClose, PanelLeftOpen, PanelRightClose, PanelRightOpen, GitFork } from 'lucide-react';
...
const STATUS_H = 36;
const PANEL_W = 320;
const CHAT_OPEN_W = 360;
const CHAT_CLOSED_W = 0;
...
export default function GraphPage() {
  ...
  const panelOpen = !!selectedNode && detailsPanelOpen;
  ...
  const paintNode = useCallback((node, ctx, globalScale) => {
    ...
    if (showLabel && isActive) {
      ...
      const labelAlpha = (isHovered || isSearched) ? 1.0 : 0.8;
      ctx.fillStyle = `rgba(255, 255, 255, ${labelAlpha})`;
      ctx.fillText(truncateLabel(node.label), node.x, node.y + radius + 2);
    }
  ...
  const handleScreenshot = useCallback(() => {
    try {
      ...
      if (!canvas) return;
      // Draw watermark on a copy
      const copy = document.createElement('canvas');
      copy.width = canvas.width;
      copy.height = canvas.height;
      const ctx2 = copy.getContext('2d');
      ctx2.drawImage(canvas, 0, 0);
      ctx2.font = '11px "Geist Mono", monospace';
      ctx2.fillStyle = 'rgba(113, 113, 122, 0.5)';
      ctx2.textAlign = 'right';
      ctx2.textBaseline = 'bottom';
      ctx2.fillText('Generated by CNI · github.com/Slambot01/CNI_ATLAS', copy.width - 12, copy.height - 12);
  ...
  if (!loading && filteredData.nodes.length === 0 && graphData.nodes.length === 0) {
    return (
      <div className="graph-page-root" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - 5.75rem)', background: T.bg }}>
        <GitFork size={48} style={{ color: T.border }} />
        <h2 style={{ fontFamily: 'var(--font-ui)', fontSize: '1.125rem', fontWeight: 600, color: T.text, marginTop: 16 }}>No codebase connected</h2>
        <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.8125rem', color: T.muted, marginTop: 8 }}>Enter a repository path above and click Analyze</p>
        <button
          onClick={() => { const input = document.querySelector('.header-path-input, input[type="text"]'); if (input) input.focus(); }}
          style={{ marginTop: 20, background: T.text, color: T.bg, fontFamily: 'var(--font-ui)', fontSize: '0.8125rem', fontWeight: 600, borderRadius: 8, padding: '10px 20px', border: 'none', cursor: 'pointer', transition: 'background 0.15s ease' }}
  ...
  return (
    <div className="graph-page-root" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 5.75rem)', overflow: 'hidden' }}>
      {/* ══════ Filter bar ══════ */}
      <div style={{ flexShrink: 0, height: 48, borderBottom: `1px solid ${T.border}`, background: T.surface, display: 'flex', alignItems: 'center', gap: 12, padding: '8px 16px' }}>
        {[
          ...
          <button key={label} onClick={() => set(!value)}
            style={{
              ...
              padding: '6px 14px',
              borderRadius: 8,
              background: value ? T.bg : 'transparent',
              border: `1px solid ${value ? T.muted : T.border}`,
              color: value ? T.text : T.muted,
              cursor: 'pointer',
              transition: 'all 0.15s ease',
            }}
            onMouseEnter={e => { if (!value) e.currentTarget.style.borderColor = T.muted; }}
            onMouseLeave={e => { if (!value) e.currentTarget.style.borderColor = T.border; }}>
            {label}
          </button>
        ))}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted }}>Min:</span>
          <input type="range" min="0" max="10" value={minConn} onChange={(e) => setMinConn(parseInt(e.target.value))}
            className="w-16 h-1 rounded-full appearance-none cursor-pointer" style={{ background: T.border }} />
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', fontWeight: 700, color: T.text, fontVariantNumeric: 'tabular-nums', width: 16 }}>{minConn}</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted }}>Color by</span>
          <select value={colorMode} onChange={(e) => setColorMode(e.target.value)}
            style={{ fontFamily: 'var(--font-ui)', fontSize: '0.8125rem', padding: '6px 12px', borderRadius: 8, background: T.bg, border: `1px solid ${T.border}`, color: T.text, cursor: 'pointer', outline: 'none', transition: 'border-color 0.15s ease' }}
            onFocus={e => { e.currentTarget.style.borderColor = T.muted; }}
            onBlur={e => { e.currentTarget.style.borderColor = T.border; }}>
            <option value="folder">Folder</option>
            <option value="importance">In-degree</option>
            <option value="default">File type</option>
          </select>
        </div>
        ...
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: T.muted, fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>
          <span style={{ color: T.text }}>{filteredData.nodes.length}</span> nodes <span style={{ color: T.muted }}>·</span> <span style={{ color: T.text }}>{filteredData.links.length}</span> edges
        </span>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0, position: 'relative' }}>
        ...
        {/* ══════ LEFT: File Details Panel ══════ */}
        {panelOpen && (
          <div style={{ width: PANEL_W, flexShrink: 0, background: T.surface, borderRight: `1px solid ${T.border}`, overflowY: 'auto', transition: 'width 0.2s ease', position: 'relative' }}>
            {/* Collapse toggle */}
            <button
              onClick={() => setDetailsPanelOpen(false)}
              style={{ position: 'absolute', top: 12, right: -14, zIndex: 20, width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: T.border, border: `1px solid ${T.border}`, borderRadius: 8, cursor: 'pointer', color: T.muted, transition: 'color 0.15s ease' }}
            ...
            <div style={{ padding: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <h3 style={{ fontFamily: 'var(--font-mono)', fontSize: '0.875rem', fontWeight: 600, color: T.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 240 }}>{selectedNode.label}</h3>
                <button onClick={handleBgClick} style={{ color: T.muted, background: 'none', border: 'none', cursor: 'pointer', padding: 4, borderRadius: 8, transition: 'color 0.15s ease' }}
                  onMouseEnter={e => { e.currentTarget.style.color = T.text; }}
                  onMouseLeave={e => { e.currentTarget.style.color = T.muted; }}>
                  <X size={16} />
                </button>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: T.muted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{shortPath(selectedNode.id)}</p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <div style={{ background: T.bg, padding: '8px 12px', borderRadius: 8, flex: 1, textAlign: 'center' }}>
                    <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted }}>IN</p>
                    <p style={{ fontFamily: 'var(--font-mono)', fontSize: '1.125rem', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: T.text }}>{selectedNode.indegree}</p>
                  </div>
                  <div style={{ background: T.bg, padding: '8px 12px', borderRadius: 8, flex: 1, textAlign: 'center' }}>
                    <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted }}>OUT</p>
                    <p style={{ fontFamily: 'var(--font-mono)', fontSize: '1.125rem', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: T.text }}>{selectedNode.outdegree}</p>
                  </div>
                  <div style={{ background: T.bg, padding: '8px 12px', borderRadius: 8, flex: 1, textAlign: 'center' }}>
                    <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted }}>TOTAL</p>
                    <p style={{ fontFamily: 'var(--font-mono)', fontSize: '1.125rem', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: T.text }}>{(selectedNode.indegree || 0) + (selectedNode.outdegree || 0)}</p>
                  </div>
                </div>
                {/* Relevance score */}
                {smartScoreMap && (smartScoreMap.get(selectedNode.id) || smartScoreMap.get(selectedNode.label)) && (
                  <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: 12, textAlign: 'center' }}>
                    <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted, marginBottom: 2 }}>Relevance</p>
                    <p style={{ fontFamily: 'var(--font-mono)', fontSize: '1.25rem', fontWeight: 700, color: T.text }}>
                      {Math.round((smartScoreMap.get(selectedNode.id) || smartScoreMap.get(selectedNode.label)) * 100)}%
                    </p>
                  </div>
                )}
                {detailsLoading ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted }}>
                    <div style={{ width: 12, height: 12, border: `2px solid ${T.border}`, borderTopColor: T.text, borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} />
                    Loading…
                  </div>
                ) : nodeDetails ? (
                  <>
                    <div>
                      <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.65rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', color: T.muted, marginBottom: 8 }}>IMPORTED BY ({nodeDetails.imported_by?.length || 0})</p>
                      <div style={{ maxHeight: 128, overflowY: 'auto' }}>
                        {nodeDetails.imported_by?.map((dep) => (
                          <button key={dep} onClick={() => handleDetailNodeClick(dep)}
                            style={{ width: '100%', textAlign: 'left', padding: '6px 8px', borderRadius: 8, fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: T.muted, background: 'none', border: 'none', cursor: 'pointer', transition: 'all 0.15s ease' }}
                            onMouseEnter={e => { e.currentTarget.style.background = T.border; e.currentTarget.style.color = T.text; }}
                            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = T.muted; }}>
                            {shortPath(dep)}
                          </button>
                        ))}
                        {(!nodeDetails.imported_by || nodeDetails.imported_by.length === 0) && <span style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted }}>(none)</span>}
                      </div>
                    </div>
                    <div>
                      <p style={{ fontFamily: 'var(--font-ui)', fontSize: '0.65rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', color: T.muted, marginBottom: 8 }}>IMPORTS ({nodeDetails.imports?.length || 0})</p>
                      <div style={{ maxHeight: 128, overflowY: 'auto' }}>
                        ...
                        {(!nodeDetails.imports || nodeDetails.imports.length === 0) && <span style={{ fontFamily: 'var(--font-ui)', fontSize: '0.75rem', color: T.muted }}>(none)</span>}
                      </div>
                    </div>
                  </>
                ) : null}
                <div style={{ display: 'flex', gap: 8, paddingTop: 12 }}>
                  <a href={`/impact?file=${encodeURIComponent(selectedNode.label)}`}
                    style={{ flex: 1, textAlign: 'center', padding: '10px 0', borderRadius: 8, fontFamily: 'var(--font-ui)', fontSize: '0.75rem', fontWeight: 500, background: T.text, color: T.bg, textDecoration: 'none', transition: 'background 0.15s ease' }}
                    onMouseEnter={e => { e.currentTarget.style.background = '#e4e4e7'; }}
                    onMouseLeave={e => { e.currentTarget.style.background = T.text; }}>
                    ⚡ Impact
                  </a>
                  <a href="/health"
                    style={{ flex: 1, textAlign: 'center', padding: '10px 0', borderRadius: 8, fontFamily: 'var(--font-ui)', fontSize: '0.75rem', fontWeight: 500, background: 'transparent', color: T.muted, border: `1px solid ${T.border}`, textDecoration: 'none', transition: 'all 0.15s ease' }}
                ...
                <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 12 }}>
                  {isBookmarked(selectedNode.label) ? (
                    <button
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 12px', borderRadius: 8, fontFamily: 'var(--font-ui)', fontSize: '0.75rem', background: 'rgba(255, 215, 0, 0.08)', border: '1px solid rgba(255, 215, 0, 0.2)', color: '#FFD700', cursor: 'pointer', transition: 'all 0.15s ease' }}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      <input
                        ...
                        style={{ width: '100%', padding: '8px 12px', borderRadius: 8, fontFamily: 'var(--font-mono)', fontSize: '0.75rem', background: T.bg, border: `1px solid ${T.border}`, color: T.text, outline: 'none' }}
                      />
                      <button
                        onClick={() => { addBookmark(selectedNode.label, bookmarkNote); setBookmarkNote(''); }}
                        style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 12px', borderRadius: 8, fontFamily: 'var(--font-ui)', fontSize: '0.75rem', background: 'transparent', border: `1px solid ${T.border}`, color: T.muted, cursor: 'pointer', transition: 'all 0.15s ease' }}
        ...
        {selectedNode && !detailsPanelOpen && (
          <button
            onClick={() => setDetailsPanelOpen(true)}
            style={{ position: 'absolute', top: 12, left: 12, zIndex: 30, width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: T.border, border: `1px solid ${T.border}`, borderRadius: 8, cursor: 'pointer', color: T.muted, transition: 'color 0.15s ease' }}
            onMouseEnter={e => { e.currentTarget.style.color = T.text; }}
            onMouseLeave={e => { e.currentTarget.style.color = T.muted; }}
          >
        ...
        {/* Graph area */}
        <div style={{ position: 'relative', flex: 1, minWidth: 400, height: graphH, background: T.bg, transition: 'all 0.2s ease' }}>
          {loading && (
